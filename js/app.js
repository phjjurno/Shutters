...
const state = {
  ...
  sameTool: null, sameToolN: 0,   // 같은 도구 연타 수 — 그로기 발동 조건
  sideT: null,                    // 피격 회피 이동 타이머
  waterReady: 0,
...
function resetGame() {
  ...
  state.sameTool = null; state.sameToolN = 0;
...
function spawnToolFx(fx, x, y) {
  ...
  } else if (fx === 'squish') {            // 볼 짜부 — 캐릭터 얼굴(볼 높이)에 맞춰 양손 압착
    const sr = stage.getBoundingClientRect();
    const cr = charBox.getBoundingClientRect();
    const w = Math.min(cr.width * 0.94, 360);
    const s = fxEl('fx-squish', cr.left - sr.left + cr.width / 2, cr.top - sr.top + cr.height * 0.39);
    s.style.width = w + 'px';
...
/* 맞을 때 가만히 서 있지 않고 살짝씩 자리를 옮기며 몸을 피한다 */
function sidestep() {
  if (state.cleared || state.escaping || state.groggy) return;
  const limX = Math.max(70, stage.clientWidth / 2 - 200);
  const cur = parseFloat(charMove.style.getPropertyValue('--tx')) || 0;
  let step = rnd(55, 130) * (Math.random() < 0.5 ? -1 : 1);
  if (Math.abs(cur + step) > limX) step = -step;
  charMove.classList.add('sidestep');
  charMove.style.setProperty('--tx', Math.round(cur + step) + 'px');
  charMove.style.setProperty('--ty', Math.round(rnd(-38, 0)) + 'px');
  clearTimeout(state.sideT);
  state.sideT = setTimeout(() => charMove.classList.remove('sidestep'), 380);
}

/* 한 대 맞았을 때 — 데미지는 power [min,max] 범위에서 랜덤 */
function landHit(tool, x, y) {
  ensureMusic();
  if (state.tool === state.sameTool) state.sameToolN++;
  else { state.sameTool = state.tool; state.sameToolN = 1; }
...
/* ---------- 그로기 — 같은 도구로만 계속 때리면 발동 ----------
   한 가지 도구로 12연타 이상 이어가면 확률적으로 그로기(3초 공격 불가).
   도구를 바꾸거나 물바가지를 쓰면 연타 카운트가 초기화된다. */
function maybeGroggy() {
  const cooled = Date.now() - state.lastGroggy > 20000;   // 최소 20초 간격
  if (state.sameToolN >= 12 && !state.groggy && !state.escaping && !state.cleared && cooled && Math.random() < 0.35) {
    state.sameToolN = 0;
...
function splashWater() {
  ...
  state.sameTool = null; state.sameToolN = 0;   // 물바가지는 같은 도구 연타 흐름을 끊는다
  /* 캐릭터가 회피 이동해 있어도 물은 캐릭터 머리 위로 떨어진다 */
  const sr = stage.getBoundingClientRect();
  const cr = charBox.getBoundingClientRect();
  ...
  for (let i = 0; i < 14; i++) {
    ...
    d.className = 'drop';
    d.style.left = (cx + rnd(-130, 130)) + 'px';
  ...
  spawnParticle(cx, stage.clientHeight * 0.3, rand(WATER.words), false, 'pc-2');
  store.waters++;
  saveStore();
